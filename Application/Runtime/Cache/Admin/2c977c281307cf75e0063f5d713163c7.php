        ...
        <?php if(is_array($MenuItem)): foreach($MenuItem as $key=>$vo): ?><li><a href="/ck_thinkphp/index.php/Admin/<?php echo ($vo["auth_c"]); ?>/<?php echo ($vo["auth_a"]); ?>"><?php echo ($vo["auth_name"]); ?></a></li><?php endforeach; endif; ?>
                ...
                <div id="Right">
                    <div class="contentArea">
                        <div class="Item hr">
                            <div class="current">添加管理员</div></div>
                        <form action="/ck_thinkphp/index.php/Admin/Access/addAdmin" method="post">
                            <table width="100%" border="0" cellspacing="0" cellpadding="0" class="table1">
                                <tr>
                                    <th width="120">登录账号：</th>
                                    <td>
                                        <input class="input" name="username" type="text"  size="40" value="" /></td>
                                </tr>
                                <tr>
                                    <th>新密码：</th>
                                    <td>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <select name="status" style="width: 80px;">
                                            ...
                                            <option value="0" >禁用</option></select>如果禁用了将无法登陆本系统</td>
                                </tr>
                                <tr>
                                    <th>所属用户组</th>
                                    <td>
                                        <select name="role_id" style="min-width: 80px;">
                                            <option value="0" disabled="disabled">超级管理员</option>
                                            <?php if(empty($role_list)): ?>请添加管理员
                                            <?php else: ?>
                                                <?php if(is_array($role_list)): $i = 0; $__LIST__ = $role_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><option value="<?php echo ($data["role_id"]); ?>">&nbsp;&nbsp;&nbsp;&nbsp;├ <?php echo ($data["role_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; endif; ?>
